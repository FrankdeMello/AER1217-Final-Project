...
class PIDController():
    ...
    def __init__(self,
                 ...
                 min_pwm: float = 20000,
                 max_pwm: float = 65535,
                 p_coeff_for=np.array([15.0, 20.0, 20.0]),#np.array([.4, .4, 1.25]),
                 i_coeff_for=np.array([20.0, 20.0, 2.0]),#np.array([.05, .05, .05]),
                 d_coeff_for=np.array([10.0, 10.0, 15.0]),#np.array([.2, .2, .5]),
...
def draw_trajectory(initial_info,
    ...
    for i in range(step, ref_x.shape[0], step):
        p.addUserDebugLine(lineFromXYZ=[ref_x[i-step], ref_y[i-step], ref_z[i-step]],
                           ...
                           lineColorRGB=colour, lineWidth=3,
                           physicsClientId=initial_info["pyb_client"])
